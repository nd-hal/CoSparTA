...
#' @param normalize Logical. If \code{TRUE}, uses \code{\link{normalize_factors}}
#'   to normalize columns to unit norm. Default \code{TRUE}.
...
#' @import ggplot2
#' @export
plot_time_factors <- function(fit, ranks = NULL, time_labels = NULL,
  ...
  ggplot(df, aes(x = time, y = value, colour = rank)) +
    geom_line(size = 0.8) +
...
#' @export
plot_channel_factors <- function(fit, ranks = NULL, channel_names = NULL,
